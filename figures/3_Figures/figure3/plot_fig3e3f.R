...
##to  display core distance as boxplots and accessory distances as violin plots fig 3e and 3f
#required packages
pkg=c("ggplot2", "phyloseq", "ape", "picante", "data.table","PMCMRplus","magick",
"ComplexHeatmap","DECIPHER","philentropy","ggtern","venn","agricolae")
...
#	dist.acc=phyloseq::distance( physeq.filter, "jaccard" , Binary=TRUE)
	...
	p3=ggplot(DT,aes(RST,coph.dist))+geom_boxplot()+geom_point(alpha=0.5)+
	theme_bw()+ylab("core genome divergence ")+theme_change
	
#	kruskal.test(data=p3$data, coph.dist ~ RST)
#	kwAllPairsConoverTest(coph.dist ~ as.factor(RST) , data=p3$data,  p.adjust.method="BH" )
#	aov.out<-aov(data=p3$data, coph.dist ~ RST)
#	print(summary(aov.out))
#	print(TukeyHSD(aov.out))
#	print(HSD.test(aov.out,trt="RST"))

	p4=ggplot(DT,aes(RST,jc.dist))+geom_jitter(alpha=0.5)+geom_violin()+
	geom_boxplot(width=0.08, outlier.shape = NA)+
	theme_bw()+ylab("accessory genome divergence")+theme_change
	
#	kruskal.test(data=p4$data, jc.dist ~ RST)
#	kwAllPairsConoverTest(jc.dist ~ as.factor(RST) , data=p4$data,  p.adjust.method="BH" )
#	aov.out<-aov(data=p4$data, jc.dist ~ RST)
#	print(summary(aov.out))
#	print(TukeyHSD(aov.out))
#	print(HSD.test(aov.out,trt="RST"))

#	pdf("Figure4C.pdf",useDingbats=FALSE)
	gridExtra::grid.arrange(p3,p4, nrow=2, ncol=2)
#	dev.off()
