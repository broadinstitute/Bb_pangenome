#!/usr/bin/env python3
"""
Generate B31 gene mappings for Panaroo output.

Adapted from: group2BB/notebook/extractGenesFromBlastHits.ipynb

Note: Panaroo FASTA headers are just gene names (e.g., >bglX),
unlike Roary which has >locus_tag gene_name format.

Creates:
  - panaroo_group_to_ID_mapping.txt (Step 3)
  - panaroo_groupID_to_BBgene.tsv (Step 4)
  - panaroo_missing_groups.tsv (Step 4)
"""
...
pg_ref = 'panaroo/output/panaroo_out/pan_genome_reference.fa'
blast_results = 'panaroo/output/panaroo_out/alignments/panaroo_vs_B31.topHit.tsv'
rs2bb = 'group2BB/B31_refseq_id_to_old_name.tsv'

# Output paths
g2idmap = 'panaroo/output/panaroo_out/alignments/panaroo_group_to_ID_mapping.txt'
g2BBmap = 'panaroo/output/panaroo_out/alignments/panaroo_groupID_to_BBgene.tsv'
mg_out = 'panaroo/output/panaroo_out/alignments/panaroo_missing_groups.tsv'

# Step 3: Create group_to_ID_mapping from pan_genome_reference.fa
# For Panaroo: header is just gene name, so ID = group
print("Step 3: Creating group_to_ID_mapping...")
with open(pg_ref) as f, open(g2idmap, 'w') as out:
    for line in f:
        if line.startswith('>'):
            gene_name = line[1:].strip()  # Remove '>' and whitespace
            # In Panaroo, gene name is both the ID and group
